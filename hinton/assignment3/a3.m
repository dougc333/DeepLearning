function a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  ...
  for optimization_iteration_i = 1:n_iters,
    ...
    fprintf('optimization_iteration_i:%d, training_batch_start:%d\n',optimization_iteration_i,training_batch_start);
    ...
    fprintf('training_batch.inputs:%dx%d training_batch.targets:%dx%d\n',rows(training_batch.inputs),columns(training_batch.inputs),
            rows(training_batch.targets),columns(training_batch.targets));
    gradient = model_to_theta(d_loss_by_d_model(model, training_batch, wd_coefficient));
    fprintf('gradient %dx%d sum:%f\n',rows(gradient),columns(gradient),sum(sum(gradient)));
    momentum_speed = momentum_speed * momentum_multiplier - gradient;
    theta = theta + momentum_speed * learning_rate;
    fprintf('theta %dx%d\n sum:%f\n', rows(theta),columns(theta),sum(sum(theta)));
...
function ret = model_to_-theta(model)
